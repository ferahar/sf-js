...
function shipClone(e,data,ship) {
    ...
    ship.forEach(block => {
        ...
        positionBlock.y = e.changedTouches[0].pageY - player.offsetTop - cell.offsetTop;
        positionBlock.x = e.changedTouches[0].pageX - player.offsetLeft - cell.offsetLeft;
...
function shipMove(e,data,shipClone) {
    ...
    for (let index = 0; index < shipClone.length; index++) {
        ...
        ship.style.left = e.changedTouches[0].pageX - player.offsetLeft - blokPosition.x +'px';
        ship.style.top = e.changedTouches[0].pageY - player.offsetTop - blokPosition.y +'px';
    };
...
function shipLocation(e,data,ship,positionStart){
    ...
    // let cell = e.changedTouches[0].target;
    let cell = document.elementFromPoint(e.changedTouches[0].pageX, e.changedTouches[0].pageY);
...
export function initiateShipMoveTouch(data) {
    ...
    let funTouch = function (e) {
        ...
        if (cell.classList.contains(data.edit)) {
            ...
            function funMouseUp(e){
                ...
                player.removeEventListener('touchmove', funMove);
                player.removeEventListener('touchend', funMouseUp);
            ...
            player.addEventListener('touchmove', funMove);
            player.addEventListener('touchend', funMouseUp);
            // player.setAttribute("onselectstart", "return false");
        }
    };

    player.addEventListener('touchstart', funTouch);
